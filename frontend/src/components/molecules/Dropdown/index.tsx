// このコンポーネントでは2つの状態をuseStateで管理
// isOpenはドロップダウンをクリックした際に選択肢のビューの表示・非表示
// selectedItemは現在選択されている選択肢を管理
// selectedItemの初期値は props の initialItemで指定
// dropdownRefはこのコンポーネントのルート要素のrefオブジェクトであり、自分自身をクリックした際ドロップダウンを閉じないようにするために参照を持っておく。
// ドロップダウンの表示は DropdownControlをクリックした際に行い、非表示は選択肢を選んだ際とコンポーネント外(document)をクリックした際に行う
// DropdownControlをクリックした際に再び非表示にならないようにhandleDocumentClickで制御
